...
def plot_error_histogram(ate_errors, output_file=None):
    """
    Histogram of Absolute Trajectory Errors.
    """
    ...
    plt.title("Distribution of Trajectory Error")
...
def plot_running_mean_error(ate_errors, output_file=None):

    window = 25

    running = []

    for i in range(len(ate_errors)):

        start = max(0, i-window)

        running.append(np.mean(ate_errors[start:i+1]))

    plt.figure(figsize=(10,5))

    plt.plot(running, linewidth=2)

    plt.grid(True)

    plt.xlabel("Frame")

    plt.ylabel("Running Mean Error (m)")

    plt.title("Accumulated ICP Drift")
...
def plot_error_heatmap(gt_positions,
                       ate_errors,
                       output_file=None):
    ...
    plt.title("Trajectory Error Heatmap")
...
def main(gt_file, est_file, output_dir=None):
    ...
    plot_error_histogram(
        ate_errors,
        output_dir / "07_error_histogram.png"
    )

    # Running mean error
    plot_running_mean_error(
        ate_errors,
        output_dir / "08_running_mean_error.png"
    )
    ...
    plot_error_heatmap(
        gt_pos,
        ate_errors,
        output_dir / "09_error_heatmap.png"
    )
    ...
    print("06_error_vectors.png")
    print("07_error_histogram.png")
    print("08_running_mean_error.png")
    print("09_error_heatmap.png")
