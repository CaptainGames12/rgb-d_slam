...
#include "open3d_slam/parameter_loaders.hpp"
...
Eigen::Isometry3d SlamWrapper::getPoseAt(const Time& timestamp) const {
  if (mapper_) {
    return mapper_->getMapToRangeSensor(timestamp);
  }
  return Eigen::Isometry3d::Identity(); // Запобіжник, якщо маппер ще не створено
}
...
void SlamWrapper::setParameterFilePath(const std::string& path) {
  ...
  try {
    ...
    if (!p.has_parent_path()) {
      // no parent: treat as filename in provided folderPath_ or current dir
    ...
    io_lua::loadParameters(folder, file, &params_);
  } catch (const std::exception& e) {
    std::cerr << "Failed to load parameter file '" << path << "': " << e.what() << std::endl;
  }
}

std::string SlamWrapper::getParameterFilePath() const {
  return parameterFilePath_;
}
